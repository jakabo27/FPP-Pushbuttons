...
Song3Playlist=$(echo $NightSong3 | cut -c1-7)
Song4Playlist=$(echo $NightSong4 | cut -c1-7)
Song5Playlist=$(echo $NightSong5 | cut -c1-7)
...
case ${STATUS} in
	...
	1 | 2)
		...
		#Song4 is running - play song 5
		elif [ "$PLAYLIST" == "$Song4Playlist" ]; then
			#Night time - play Song5 with standby
			if [ $CurrentHour -lt $OffHour -a $CurrentHour -ge $OnHour ]; then
				fpp -c stop
				fpp -p "${NightSong5}"
			#Day time or really late - play Song4 once then turn off lights
		...
		elif [ "$PLAYLIST" == "$Song5Playlist" ]; then
